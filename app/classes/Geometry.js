export default class Geometry {
  ...
  static getSightPolygon (sightX, sightY, uniquePoints, segments) {
    ...
    for (var j = 0; j < uniqueAngles.length; j++) {
      ...
      for (var i = 0; i < segments.length; i++) {
        var intersect = Geometry.getIntersection(ray, segments[i]);
  ...
  static drawPolygon (ctx, relative, polygons, fillStyle) {
    ctx.fillStyle = fillStyle;
    ctx.beginPath();
    ctx.moveTo(polygons[0].x - relative.x, polygons[0].y - relative.y);
    for (var i = 1; i < polygons.length; i++) {
      var intersect = polygons[i];
      ctx.lineTo(intersect.x - relative.x, intersect.y - relative.y);
    }
    ctx.fill();
  }
